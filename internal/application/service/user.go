...
func (s *userService) Login(ctx context.Context, req *types.LoginRequest) (*types.LoginResponse, error) {
	...
	account := normalizeLDAPAccount(req.Email)
	profile, ldapErr := s.authenticateWithLDAP(ctx, account, req.Password)

	var user *types.User
	var err error
	if ldapErr == nil {
		user, err = s.getOrCreateLDAPUser(ctx, profile)
		if err != nil {
			logger.Errorf(ctx, "Failed to sync LDAP user: %v", err)
			return &types.LoginResponse{
				Success: false,
				Message: "LDAP login failed",
			}, nil
		}
		logger.Info(ctx, "LDAP login and user sync successful")
	} else {
		logger.Warnf(ctx, "LDAP authentication failed, fallback to local login: %v", ldapErr)

		// Local account fallback (existing behavior)
		user, err = s.userRepo.GetUserByEmail(ctx, req.Email)
		if err != nil || user == nil {
			logger.Warn(ctx, "User not found for email")
			return &types.LoginResponse{
				Success: false,
				Message: "Invalid email/account or password",
			}, nil
		}
		...
		logger.Info(ctx, "Local password verification successful")
	}
