...
type InviteMemberRequest struct {
	UserID     string        `json:"user_id"`    // 单个用户ID
	Department string        `json:"department"` // 部门（来源于 user.avatar）
	Action     string        `json:"action"`     // add/remove，默认 add
	Role       OrgMemberRole `json:"role"`       // add 时可选，默认 viewer
}
